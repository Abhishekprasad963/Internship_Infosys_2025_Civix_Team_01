...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      console.log("📧 Sending forgot password OTP to:", forgotPasswordData.email);
      const response = await API.forgotPassword(forgotPasswordData.email);
      console.log("✅ Forgot password response:", response.data);
      setForgotPasswordMessage(response.data.message || "OTP sent. Please check your inbox.");
      if (currentStep === 1) setCurrentStep(2);
    } catch (err) {
      console.error("❌ Forgot password error:", err);
      // Even if there's an error, proceed to OTP step for security
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      console.log("🔍 Verifying OTP for password reset:", forgotPasswordData.email);
            ...
            <div className="text-center">
              ...
              {forgotPasswordMessage && (
                <div className={`mb-4 p-3 rounded-lg text-sm ${
                  forgotPasswordMessage.includes("successfully") ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"
                }`}>
                  {forgotPasswordMessage}
                </div>
              )}
              {forgotPasswordError && (
                <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-lg text-sm">{forgotPasswordError}</div>
              )}
              ...
              <form onSubmit={currentStep === 1 ? handleSendOtp : currentStep === 2 ? handleVerifyOtp : handleResetPassword}>
                ...
                {currentStep === 2 && (
                  <input 
                    ...
                    required 
                    maxLength="6"
                    className="w-full px-4 py-2 border rounded-lg mb-4 text-center text-lg font-mono" 
                  />
                )}
                {currentStep === 3 && (
                  <>
                ...
                <div className="flex space-x-3 mt-4">
                  <button 
                    type="button" 
                    onClick={() => currentStep > 1 ? setCurrentStep(currentStep - 1) : handleForgotPasswordModalClose()} 
                  ...
                  <button 
                    ...
                    className="flex-1 py-2 px-4 bg-green-600 text-white rounded-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {forgotPasswordLoading ? "Processing..." : (currentStep === 1 ? "Send OTP" : (currentStep === 2 ? "Verify OTP" : "Reset Password"))}
                  </button>
                </div>
              </form>
            </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="relative">
              ...
              <button 
                ...
                className="absolute inset-y-0 right-0 flex items-center px-3 pt-6"
              >
            ...
            <button 
              type="submit" 
              disabled={isLoading} 
              className="w-full py-2 bg-green-400 text-white font-semibold rounded-lg hover:bg-green-500 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
